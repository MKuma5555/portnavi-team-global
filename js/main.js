...
(function () {
  ...
  function closeDrawer(returnTo) {
    ...
    (returnTo || lastTrigger || openBtn)?.setAttribute('aria-expanded', 'false');
    // フォーカスも元のトリガーへ返す
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    document.body.addEventListener('click', (e) => {
      const trigger = e.target.closest('[data-action="open-share"], .share-open');
      if (!trigger) return;
    ...
    icons.forEach(a => {
      a.addEventListener('click', async (e) => {
        ...
        switch (a.dataset.share) {
          case 'line':
            popup(`https://social-plugins.line.me/lineit/share?url=${url}`, 'line');
            break;
          ...
          case 'x':
            // X (Twitter)
            popup(`https://twitter.com/intent/tweet?url=${url}&text=${title}`, 'x');
            break;
    ...
    function popup(u, name) {
      ...
      window.open(u, name, `width=${w},height=${h},left=${x},top=${y},resizable,scrollbars`);
    }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('.like-proxy').forEach(proxy => {
      ...
      if (realCountEl) {
        ...
        mo.observe(realCountEl, { childList: true, subtree: true, characterData: true });
      }
      ...
      function syncState() {
        ...
        if (realBtn) {
          const liked = realBtn.classList.contains('wp_ulike_btn_is_active') || realBtn.classList.contains('wp_ulike_btn_liked');
